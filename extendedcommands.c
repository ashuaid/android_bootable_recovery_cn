...
void show_install_update_menu()
{
    ...
    char* install_menu_items[] = {  "从SD卡中选择ZIP格式刷机包",
                                    NULL,
                                    "选择多个ZIP格式刷机包刷入",
                                    "选择/sdcard/update.zip刷机",
                                    "使用ADB从电脑推送刷机",
                                    "切换刷机包签名验证",
                                    "设置默认的根目录文件夹",
                                    NULL };
    ...
    if (volume_for_path("/emmc") != NULL) {
        other_sd = "/emmc/";
        install_menu_items[1] = "从内置SD卡选择ZIP文件";
    }
    else {
        other_sd = "/external_sd/";
        install_menu_items[1] = "从外置SD卡选择ZIP文件";
    }
...
void show_choose_zip_menu(const char *mount_point)
{
    ...
    static char* headers[] = {  "选择ZIP格式刷机包安装",
                                NULL
    };
...
void show_partition_menu()
{
    ...
    for (;;)
    {
        ...
        if (!is_data_media()) {
            options[mountable_volumes + formatable_volumes] = "启用U盘模式挂载";
            options[mountable_volumes + formatable_volumes + 1] = NULL;
        } else {
            options[mountable_volumes + formatable_volumes] = "格式化 /data and /data/media (/sdcard)";
            options[mountable_volumes + formatable_volumes + 1] = "启用U盘模式挂载";
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择还原镜像",
                                "",
                                "先选择一个需要还原的镜像",
                                "下级菜单将会显示更多选项",
                                "",
...
void format_sdcard(const char* volume) {
    ...
    if (chosen_item == GO_BACK)
        return;
    if (!confirm_selection( "确认格式化吗？", "是 - 格式化该设备"))
        return;
...
void show_advanced_power_menu() {
    static char* headers[] = { "高级重启菜单", "", NULL };
...
{
    static char* headers[] = {  "高级设置选项",
                                NULL
    };

    char item_datamedia[35];
    char* list[] = {    "清空Dalvik Cache",
                        "保存日志",
                        "按键测试",
                        "显示日志",
                        item_datamedia,
    ...
    for (;;)
    {
        if (is_data_media()) {
            if (use_migrated_storage())
                sprintf(item_datamedia, "切换显示内置SD卡无0目录");
            else sprintf(item_datamedia, "切换显示内置SD卡有0目录");
        }
...
void process_volumes() {
    ...
    ui_print("将会进行备份和还原操作.\n");
    ui_print("如果出现错误你可以还原\n");
    ui_print("文件名为%s的备份\n", backup_name);
    ui_print("\n");
...
void handle_failure(int ret)
{
    ...
    ui_print("Log日志已经被复制到 /sdcard/clockworkmod/recovery.log\n");
}
...
int verify_root_and_recovery() {
    ...
    if (0 != lstat("/system/etc/.installed_su_daemon", &st)) {
        ...
        if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
            if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("确认禁止还原官方恢复模式?", "是 - 禁止还原到官方恢复系统")) {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("系统没有发现Root权限. 进行Root?", "是 - 进行Root (/system/xbin/su)")) {
